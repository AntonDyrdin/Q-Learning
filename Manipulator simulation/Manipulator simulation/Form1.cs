...
namespace Manipulator_simulation
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            //  DMS.addParameter("f4", "0-45,45-90,90-135,135-180,180-225,225-270,270-315,315-360");
                ...
                for (int it = 0; it < 10; it++)
                {
                    ...
                    while (e > 20)
                    {
                        /* point = new Point(MousePosition.X, MousePosition.Y);
                         point.Offset(-this.Location.X - 13-picBox.Location.X, -(this.Location.Y + 38) - picBox.Location.Y);  */
                        ...
                        {
                            DMS.setQ((500 - e) / 500);
                        MPV.addPoint((500 - e) / 500, "reward");
                        }
                        else
                        {
                            DMS.setQ((-e) / 500);
                        MPV.addPoint((-e) / 500, "reward");
                        }
                     MPV.addPoint(DMS.lastAction.Q, "Q");

                        if (DMS.ActualState != DMS.lastState)
                            MPV.parameters[2].functions[0].points[MPV.parameters[2].functions[0].points.Count - 1].mark = "S[" + DMS.S.IndexOf(DMS.ActualState).ToString() + "]";

                        if (DMS.lastAction.type != lastActionType)
                            MPV.parameters[2].functions[0].points[MPV.parameters[2].functions[0].points.Count - 1].mark = DMS.lastAction.type;

                        lastActionType = DMS.lastAction.type;
                    ...
                    MPV.parameters[0].addPoint(steps, "steps[" + i.ToString() + "]");
